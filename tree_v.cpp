...
struct intermittent{
  ...
  void compareAll(const term& basemask, std::vector<implicant>& primes,std::map<term,intermittent*>& itmList){
...
void recTreeConstructor(bdt node,const std::vector<implicant>& primes, const term& nodeRemains);
bool is1(char c);
